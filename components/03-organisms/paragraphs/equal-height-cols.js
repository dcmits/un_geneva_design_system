  ...
  //});
    jQuery('.row.with-cols>div>.field_un_column_one--wrapper>.field_un_column_one:first-child>.un-box:first-child').each(function(){
      ...
      if(jQuery(this).parent().parent().parent().parent().children().length == 3) {
        let sibling = jQuery(this).parent().parent().parent().next();
        if(sibling && sibling.find('.field_un_column_two--wrapper').children().length == 1 && sibling.find('.field_un_column_two--wrapper>.field_un_column_two:first-child>.un-box').length == 1) {
          sibling.find('.field_un_column_two--wrapper>.field_un_column_two:first-child>.un-box:first-child').each(function(){
            let col2 = jQuery(this);
            let sibling2 = sibling.next();
            console.log(sibling2);
            if(col1 && col2 && sibling2.find('.field_un_column_three--wrapper').children().length == 1 && sibling2.find('.field_un_column_three--wrapper>.field_un_column_three:first-child>.un-box').length == 1) {
              sibling2.find('.field_un_column_three--wrapper>.field_un_column_three:first-child>.un-box:first-child').each(function(){
